  ...
  const runChecks = useCallback(async () => {
    ...
    try {
      ...
      results[results.length - 1] = {
        ...
        icon: "wifi",
        status: r ? "ok" : "fail",
        detail: r ? "Online" : "Offline â sem internet",
      };
    } catch {
    ...
    if (API_BASE) {
      try {
        ...
        if (r?.ok) {
          results[results.length - 1] = { id: "api", label: "Servidor da API", icon: "server", status: "ok", detail: `OK Â· ${API_BASE}` };
        } else {
      ...
      results[results.length - 1] = { id: "api", label: "Servidor da API", icon: "server", status: "warn", detail: "EXPO_PUBLIC_DOMAIN nÃ£o configurado" };
    }
    setItems([...results]);
    ...
    if (API_BASE) {
      try {
        ...
        results[results.length - 1] = {
          ...
          status: r?.ok ? "ok" : "warn",
          detail: r?.ok ? "Respondendo normalmente" : `HTTP ${r?.status} â verifique o servidor`,
        };
      } catch {
        results[results.length - 1] = { id: "ai", label: "IA (Cortesia Gemini)", icon: "cpu", status: "fail", detail: "Endpoint /api/ai/chat nÃ£o acessÃ­vel" };
      }
    } else {
      results[results.length - 1] = { id: "ai", label: "IA (Cortesia Gemini)", icon: "cpu", status: "warn", detail: "DomÃ­nio do servidor nÃ£o configurado" };
    }
    setItems([...results]);
    ...
    results.push({
      ...
      detail: activeAI ? `${activeAI.name} Â· ${activeAI.model || "modelo padrÃ£o"}` : "Nenhum provedor configurado â use Cortesia Gemini",
    });
    setItems([...results]);
    ...
    results.push({
      ...
      icon: "terminal",
      status: "ok",
      detail: `${terminalSessions.length} sessÃ£o(Ãµes) Â· Node.js Real disponÃ­vel via botÃ£o [NODE]`,
    });
    setItems([...results]);
    ...
    results.push({
      ...
      detail: dbConfigs.length > 0
        ...
        : "Nenhum banco configurado â adicione em ConfiguraÃ§Ãµes",
    });
    setItems([...results]);
    ...
    results.push({
      ...
      detail: activeProject
        ? `${activeProject.name} Â· ${activeProject.files.length} arquivo(s)`
    ...
    results.push({
      ...
      icon: "hard-drive",
      status: "ok",
      detail: `Projetos: ${activeProject?.files.length || 0} arquivos Â· AsyncStorage ativo`,
    });
    setItems([...results]);
      ...
      <View style={[styles.overlay]}>
        <View style={[styles.sheet, { backgroundColor: colors.background, borderTopColor: colors.border }]}>
          <View style={[styles.header, { borderBottomColor: colors.border }]}>
            <Text style={[styles.title, { color: colors.foreground }]}>ð©º Status do Sistema ao Vivo</Text>
          ...
          {lastUpdate && (
            <View style={[styles.summaryBar, { backgroundColor: colors.card }]}>
              <Text style={[styles.summaryItem, { color: "#00d4aa" }]}>â {counts.ok} ok</Text>
              <Text style={[styles.summaryItem, { color: "#f59e0b" }]}>â ï¸ {counts.warn} atenÃ§Ã£o</Text>
              <Text style={[styles.summaryItem, { color: "#ef4444" }]}>â {counts.fail} falha</Text>
